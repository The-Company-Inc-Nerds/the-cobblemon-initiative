...
execute if score #stk_ok cd_calc matches 1.. run kill @e[tag=ci_anchor_sq_hz_analyst]
execute if score #stk_ok cd_calc matches 1.. at @s run summon minecraft:armor_stand ~1.5 ~ ~ {Invisible:1b,NoGravity:1b,Tags:["ci_anchor_sq_hz_analyst"]}
execute if score #stk_ok cd_calc matches 1.. run tbcs attach rctmod:sq_hz_analyst @e[tag=ci_anchor_sq_hz_analyst,limit=1]
execute if score #stk_ok cd_calc matches 1.. run tbcs battle GEN_9_SINGLES @s vs rctmod:sq_hz_analyst onwin {1: ['kill @e[tag=ci_anchor_sq_hz_analyst]', 'cobbledollars give @1 300', 'tag @1 add defeated_sq_hz_analyst', '@2 say Refund issued, doubled, logged as a confidence event. The model did not see you coming. Neither did the model maker.', 'execute as @1 run function cobblemon_initiative:sidequest/minutes/billable_hours', 'execute as @1 run function cobblemon_initiative:economy/wager_sweetener'], 2: ['kill @e[tag=ci_anchor_sq_hz_analyst]', '@1 say The stake stays with the house. Customer confidence: measurably improved. Thank you for participating in the figures.']}
