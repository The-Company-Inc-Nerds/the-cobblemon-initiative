...
# Run AS the player from Sayuris donate button (museum assistant by the bone statue —
# never Tamiko; accidental name glitches poison the erasure motif). clear with maxCount 0
...
execute unless score @s ci_sq_scratch matches 6.. run tellraw @s [{"text":"Sayuri counts the specimens twice. Six assorted bones complete the exhibit set — keep brushing.","color":"gray"}]
