# Curator Kenji resurrection machine - Dome Fossil -> Kabuto, on the museum platform (top 1902.5/115/
# 2313.8; the fossil floats at +1 = 1902.5/116/2313.8). Run AS THE PLAYER from the revive_kabuto dialog
# button (gated not_tag sq_revived_kabuto - per-species, so each dug fossil revives once). Consume-probe
# idiom: `clear @s <item> 0` counts without removing; only latch + float + begin if the player holds
# one. Carry tags: shared ci_reviving (generic mid/finish beats) + ci_reviving_kabuto (finish branches
# on it to spawn the right revived_kabuto gift body). Kept as a literal turnin (not a macro) so
# dialog_lint sees the sq_revived_kabuto grant.
...
execute if score #turnin ci_item matches ..0 run title @s actionbar [{"text":"You have no Kabuto to revive.","color":"red"}]
...
execute if score #turnin ci_item matches 1.. positioned 1902.5 116 2313.8 run summon minecraft:item ~ ~ ~ {Item:{id:"cobblemon:dome_fossil",count:1},NoGravity:1b,Invulnerable:1b,PickupDelay:32767s,Age:-32768s,Tags:["ci_fossil_float"],CustomName:'{"text":"Kabuto","color":"gold"}'}
