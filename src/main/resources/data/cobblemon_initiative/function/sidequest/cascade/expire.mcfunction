...
# plunge pools, so the runner simply walks back down. Free retries at Ayame.
title @a[tag=ci_ascending] title [{"text":"TIME EXPIRED","color":"red","bold":true}]
title @a[tag=ci_ascending] subtitle [{"text":"Ayame resets the clock - free retries","color":"gray"}]
