...
| **Dialog condition sets stop ANDing somewhere past 2 conditions** (LIVE-observed 2026-07-19, not yet bytecode-verified: the Warden's 9-condition `all_tags` gate — all 9 present in the compiled preset — gated IN with one required tag absent + two stripped; a 2-condition `all_tags` gate is live-PROVEN to AND correctly, kalahar survey stone 3) | Treat `all_tags` beyond ~2 entries as unreliable for anything story-critical: lower big gates to a single derived band tag (the §3 band_tags pattern) instead. RULED acceptable for the one shipped user (the Deep Dark Warden door — post-game order/gating waived by showrunner 2026-07-19); decompile `ConditionDataSet`/dialog evaluation before ever relying on wide ANDs |
...
| **`easy_npc dialog open` on a JUST-SPAWNED body silently NPEs client-side** (bytecode+log-verified, 6.25.0): the server path is synchronous+correct (`PresetHandler.createAndImportNewEntity` runs `importPresetData` BEFORE `addFreshEntity`; `DialogCommand.openDialog`→`DialogDataSet.hasDialog` label match→`MenuManager.openMenu`; datapack source has GAMEMASTERS=2 so `AccessSecurity` grants ADMIN_OVERRIDE — no filtering). The break is CLIENT-side: the client rebuilds the menu from `LivingEntityManager.npcEntityMap` (populated only on `ClientEntityEvents$Load`), and `DialogScreen.<init>` (DialogScreen.java:89) calls `getEasyNPC().getLivingEntity()` with NO null guard. A body spawned this tick via `import_new` isn't client-tracked yet when the open-menu packet lands → null → NPE on the render thread → screen silently never builds (log: `EasyNPC entity … is missing for menu`). Same NPE fires for a PERSISTED npc the instant the player tp's out of its tracking range → the hard requirement is "entity currently client-tracked", which persisted/latch NPCs satisfy but per-call spawns race. Dialog LABELS are normalized at load (`generateDialogLabel` = trim+lowercase+space→`_`+strip `[^a-z0-9_]`+trunc 32) so any uppercase/`.`/space in a `Label` silently won't match a command arg. NO server-side defer (5t/20t both fail) fixes the race | Open a freshly-spawned body's dialog via a BOUNDED RETRY, not a fixed delay: re-issue `easy_npc dialog open` every ~2t until it lands, capped (the PokéPhone used a 4-try `#phone_open_tries` budget in `phone/deliver`+`phone/open` until 0.7.0-a20 moved calls off Easy NPC dialogs onto the mod's own call screen — the fact still binds any future just-spawned-body open). Keep the budget SMALL — re-opening restarts the `DialogScreen` typewriter reveal. Bodies spawned at the player's feet track in ~1-2t. The class-of-bug alternative: use a PERSISTENT always-loaded caller NPC (swap dialog/skin per use) so it's always tracked and the open never NPEs |
| **`MoveBackToHomeGoal.reachedHome()` NPE-crashes FLYING bodies** (bytecode, alpha.2 — this killed the marsh playtest server): the no-home branch calls `getGroundPathNavigation().isDone()` with NO null check (MoveBackToHomeGoal.java:128), `hasHomePosition()` treats the preset default Home {0,0,0} as "no home" (BlockPos.ZERO check), and `getGroundPathNavigation()` is an instanceof-GroundPathNavigation cast that returns null for flying navs — `easy_npc:fairy` creates FlyingPathNavigation, `easy_npc:allay` inherits vanilla Allay flying nav (bogged/cobblemon_npc keep ground nav, safe). canUse() reaches it behind nextInt(240), so the crash lands ~12 s median after the chunk loads. The goal ships from the `ambient_wander`/`ambient_guard_patrol`/`duel_melee` snippets (MOVE_BACK_TO_HOME) | Never author a home-based movement objective on a fairy/allay etype — `content_compile` hard-errors on it since alpha.2, and `MoveBackToHomeGoalMixin` (remap=false, `modCompileOnly` on the pinned Modrinth artifact) guards live worlds by returning "already home" when ground nav is null: mandatory because a persisted body re-registers the saved goal on entity load and preset re-import orphans-but-never-stops it. Re-verify the shadowed `navigationData` field + private `reachedHome` on any Easy NPC bump |
