...
# raid-gate math (badges 7, fields 6 - alpha.26 audit rulings: SIX-field canon, owned by
# 10_hq_raid.md; matches DJ gate + quest/render + liberation/ceremony), points at Fenceline/Coldfurrow, and
...
execute unless entity @s[tag=hq_pointer_done] run title @s subtitle [{"text":"badges seven, fields six, then the tower.","color":"gray"}]
execute unless entity @s[tag=hq_pointer_done] run tellraw @s [{"text":"THE DOOR DOWNTOWN - ","color":"gold","bold":true},{"text":"the tower door opens on seven badges and six liberated fields. Starve the last feeders first: Fenceline Acres south, Coldfurrow east on the Ryujin road. Then walk into the lobby at ","color":"gray"},{"text":"[1590 51 1028]","color":"red"},{"text":".","color":"gray"}]
