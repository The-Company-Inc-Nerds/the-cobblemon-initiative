# Quests: Gaviota Port

A working harbor where the Company runs the docks and the tariffs. The CobbleDollar is openly
slipping now — grain is pushed hardest here, and the port's wheat-handlers have stopped being
polite. Home of the Daycare and the Water Gym.

**Status:** ✅ Done · 🚧 WIP (partial) · ❌ Not yet implemented — as of the 2026-07-21 audit.

> [!NOTE]
> **Content status:** the **gym** (Leader Neptune + the Tide Clock), the
> [[Guidebook Facilities]], and Gaviota's three tracked **town side-quests** (Mending the
> Deep Nets, The Tide Market, Adjusted Freight) are shipped and on the quest tracker
> (slots 44–46). The non-tracked port beats (Wheat by Sea, the rumor hub, dock flavour) run
> alongside them.

---

## Quest index

| Quest | Type | Giver / gate | Status |
|-------|------|--------------|:------:|
| **Gym 5 — Neptune (Water Badge)** | Gym | Tide arena | ✅ shipped |
| **Daycare** | Facility | Daycare Keeper | ✅ shipped |
| Town side-quests | Side | see **Side Quests** below | ✅ shipped |

---

## Gym 5 — The Tide Gym (Water Badge 🌊)

> **Leader Neptune** · tide arena at `[624 82 3536]` · **cap → 50** · **memory fragment 5**

**Entry cap: 44** (from the Fighting Badge). Neptune's ace sits at **46** — two over.

### The gimmick — the Tide Clock

The arena runs a **high/low tide cycle on a ~4-minute timer**. Neptune fields a **different
rain-variant team depending on the tide** at the moment the battle opens — so *when* you start the
fight is part of the fight. Watch the water line before you challenge.

### The ladder

| Rung | Who | Team |
|------|-----|------|
| Sailor Marco | trainer_1 | Tentacool 38, Carvanha 39 |
| Swimmer Coral | trainer_2 | Staryu 39, Shellder 39 |
| Fisherman Ivan | trainer_3 | Goldeen 39, Remoraid 39 |
| Surfer Paz | trainer_4 | Wailmer 39, Clamperl 39 |
| Jr. Apprentice Tide | jr_apprentice | Tentacruel 39, Azumarill 40 |
| Apprentice Marina | apprentice | Tentacruel 42, Sharpedo 42, Starmie 41 |
| **Leader Neptune** | leader | **Cloyster 44, Starmie 44, Lapras 45, Gyarados 46 (ace)** |

> Electric and Grass are the answer, but Gyarados (Water/Flying) laughs at Electric-only leads —
> bring Grass or a Rock/Electric mix.

### Rewards

Standard badge packet: flat prize, **cap → 50**, **memory fragment 5**, next Pokémart tier.

---

## Side Quests

> [!NOTE]
> The three tracked side-quests below — **Mending the Deep Nets**, **The Tide Market**, and
> **Adjusted Freight** — are shipped and appear on the quest tracker once the rumor hub
> (Nurse Coralie) has introduced the town. The rest of the table is non-tracked port texture:
> the paid heal / rumor board, the Wheat-War ambush at Westwind, and pure flavour.

The Company runs Gaviota's docks and its tariffs, and by gym 5 the CobbleDollar is openly slipping — dock pays come up light, sea-freight tariffs adjust in the harbour's favour, and grain has started moving by hull. A few port veterans place your face (the Company has you filed); the union rep half-remembers a name on the port's founding charter, not the person; ordinary dockhands just feel the money getting shorter.

| Status | Quest | Giver | What you do | Reward |
|:------:|-------|-------|-------------|--------|
| ✅ Done | **What's the Word on the Docks** | Nurse Coralie | Paid full-party heal + a rumor board pointing at the port's open work | Paid heal (rides the index); no cash |
| ✅ Done | **Mending the Deep Nets** | Netmender Bosun Rui **@ 605 93 3660** | Bring 8 string to restring the deep-sea nets, then an optional friendly wager | 300 CD + a training gift (exp candy); 200 CD wager |
| ✅ Done | **The Tide Market** | Fence Odessa **@ 552 81 3552** | Recover a seized crate off the customs float to unlock her black-market stock | Black-market shop access + 150 CD |
| ✅ Done | **Adjusted Freight** (The Manifest Audit) | Dockmaster Kaito **@ 566 84 3560** | Cross-check three freight manifests against the real barrel counts | 400 CD union honorarium |
| ✅ Done | **Wheat by Sea (Westwind)** | Tidewatch Sable **@ 700 103 3255** | Cut the grain barges at Westwind — trade turns to an ambush; the win frees the field | 500 CD + Westwind Fields liberated |
| ✅ Done | Dock Hand Mattias | *(flavour)* | Talk to a restructured-pay dockhand — the human face of the tariff | None (flavour) |

### What's the Word on the Docks

**Nurse Coralie** runs the Port Center counter and doubles as the harbour's rumor hub. She heals your team at the posted rate — a fee that rides the instability index like every nurse on the route. Ask what the word on the docks is and she points you at one of the town's open quests, or gives you the freight talk when nothing's pending. No battle, no cash; the heal and the pointers are the service.

### Mending the Deep Nets

**Netmender Bosun Rui** is the retired boatswain of Gaviota's deep-sea fleet; his big nets rotted and he won't fish until they're restrung. Bring **8× string** (dropped by wharf/boardwalk spiders after dark) and he restrings two nets — keeps one, hands you **300 CD** and a standard training gift (exp candy, scaled to your badge count). Then, one boatswain to a promising deckhand, he offers a **friendly 200 CD wager** against two Water types at Lv 52 (two over your cap). Entirely opt-in — take it, or wave it off for a **100 CD** non-engagement fee (a loss costs the 200 stake, fail-soft).

### The Tide Market

Under the fish-market boardwalk, **Fence Odessa** fences the goods the Company's tariff prices out of reach — *"theft was the tariff; this is just the water correcting a bad ledger."* She's guarded until you prove you're not customs: recover a **crate the customs float seized** off the far slip, and her black-market slip opens to you (plus an optional finder token). A grey but sympathetic beat, since the Company made the shortage. Declining just leaves the market locked; it's a bonus, not a gate.

### Adjusted Freight

**Dockmaster Kaito**, the harbour-union foreman, is being told the barrels he counts are worth less than the manifests say — tariffs adjusted, always in the Company's favour. He asks you to **cross-check three freight manifests** against the real barrel counts at three wharf points; every gap is money that left the port without a boat. Report all three for a **400 CD** honorarium (shorted by the same rate it audits — the joke is deliberate). Kaito half-recognises the *signature scraped off the port's founding plaque*, and tells you you have the look of that name — a foolish thing to say to a stranger.

### Wheat by Sea (Westwind)

The monopoly now ships grain by hull, and **Westwind Fields** has a coastal loading dock east along the coast. **Tidewatch Sable** is the handler running the barges; she'll first sell you the alternative-currency pitch, but once you've freed enough fields across the region she places your face mid-trade (*"you're supposed to be filed, gone"*) and turns hostile. **Beating her ambush liberates Westwind Fields** — easing prices, flipping the zone banner, and firing the LIBERATED title card. The ambush is tuned cap-legal, and the trade is always walk-away-able; only the recognition fight is forced, and only once you're deep enough into the Wheat War for her to know the face.

### Dock Hand Mattias

A flavour node on the wharf: twenty years on the docks, same barrels and same tide for half the pay — *"restructured, they call it."* Mattias never places your face; he just feels the money getting quietly shorter and the whole port smiling about it. No battle, no reward — pure back-echo, with a soft nod east toward Kalahar Reach. Free a field anywhere and he'll mention the till breathed easier for a day.

---

## Where it sits on the road

The economy voice hardens here — instability toward 40, tariff gouging, grain pushed hardest. The
Daycare (2 custody slots, cap-clamped XP drip) is a Gaviota fixture — see
[[Guidebook Facilities]]. Next: **Kalahar Reach** (Ground, gym 6). Gimmicks:
**[[Guidebook Gym Mechanics]]**.
