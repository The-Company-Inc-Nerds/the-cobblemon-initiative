# Sango Classic — the bar emptied. Lose only the entry; Deka invites re-entry. Never a punishment.
...
tellraw @a[tag=classic_active] [{"text":"Deka declares the quarter soft. The pond keeps no other score — enter again whenever the water calls.","color":"yellow"}]
