...
tellraw @s [{"text":"Quietly: ","color":"aqua"},{"text":"The marsh runs deeper than the gym. Below the water a drowned stair winds down to a light that does not warm - a shrine older than the town, and a priestess at its floor who turns away all but one tread. They say she has been waiting. It is south across the deep water, near where the ferry poles run.","color":"gray"}]
