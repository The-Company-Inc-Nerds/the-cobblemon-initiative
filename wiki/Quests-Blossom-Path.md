...
```mermaid
flowchart TD
    W["Approach the tent<br/>(one-shot warning at ~30 blocks)"] --> M{"How do you get Memo 44-C?"}
    M -->|"Stealth — 8 s unseen<br/>inside the 14-block ring"| E["Voices at the tent flap:<br/>3-page eavesdrop, then lift the<br/>memo from the dispatch tray — unnoticed"]
    M -->|"Fight both agents"| F["Sani 230 CD + Haruki 240 CD (flat)<br/>agents gone for good — the memo<br/>is in the abandoned paperwork"]
    E --> L["Deliver to Lucian in Sango<br/>[2626.5 118 2776.5]"]
    F --> L
    L --> R["400 CD (Verified Rate)<br/>+ 2× Super Potion + standard pack"]
```
