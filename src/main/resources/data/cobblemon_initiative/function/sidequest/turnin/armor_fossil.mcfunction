...
# dialog_lint sees the sq_revived_shieldon grant.
...
execute store result score #turnin ci_item run clear @s cobblemon:armor_fossil 0
execute if score #turnin ci_item matches ..0 run title @s actionbar [{"text":"You have no Shieldon to revive.","color":"red"}]
execute if score #turnin ci_item matches 1.. run clear @s cobblemon:armor_fossil 1
execute if score #turnin ci_item matches 1.. run tag @s add sq_revived_shieldon
execute if score #turnin ci_item matches 1.. run tag @s add ci_reviving
execute if score #turnin ci_item matches 1.. run tag @s add ci_reviving_shieldon
execute if score #turnin ci_item matches 1.. positioned 1902.5 116 2313.8 run summon minecraft:item ~ ~ ~ {Item:{id:"cobblemon:armor_fossil",count:1},NoGravity:1b,Invulnerable:1b,PickupDelay:32767s,Age:-32768s,Tags:["ci_fossil_float"],CustomName:'{"text":"Shieldon","color":"gold"}'}
