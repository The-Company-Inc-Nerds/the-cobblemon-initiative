...
**Trader tug-of-war.** `wheat_trader/tick` reads the shared `fields_liberated` counter and escalates traders in one-way, relog-safe tiers: 0–1 fields → trade only; **2+ → recognition** (`wheat_trader_suspicious`); **4+ → ambush** (`wheat_trader_hostile`, the post-trade battle). Field Liberation (P4) is the system that increments `fields_liberated`; the `wheat_war_active` flag and field wiring are in progress per the project TODO, so this is the designed-but-partially-wired branch of the economy.
