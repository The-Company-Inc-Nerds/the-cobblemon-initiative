 ...
 */
public class DevNoteInit implements ModInitializer {
  ...
  private static void selectEntity(Player player, Entity entity) {
    ...
    player.sendSystemMessage(Component.literal(
      ...
      + "\n§7  /npcnote note <text>, /npcnote move (or right-click a block), /npcnote log"));
  }
  ...
  public static int smokeList(Player player) {
    ...
    player.sendSystemMessage(Component.literal(
      "§7Use §f/ci smoke next §7for the first unmarked item, or §f/ci smoke show <id>§7."));
  ...
  public static int smokeNext(Player player) {
    ...
    for (SmokeItem it : SMOKE_ITEMS) {
      if (!results.containsKey(it.id)) {
        ...
        player.sendSystemMessage(Component.literal(
          "§7  /ci smoke pass " + it.id + " §8| §7comment " + it.id
          + " <note> §8| §7fail " + it.id + " <note>"));
        return 1;
      }
    }
    player.sendSystemMessage(Component.literal(
      "§aAll " + SMOKE_ITEMS.size() + " smoke items marked. §7Run §f/ci smoke log§7 to dump them."));
    return 0;
  }
  ...
  public static boolean smokeMark(Player player, String id, String status, String note) {
    ...
    if (it == null) {
      player.sendSystemMessage(Component.literal(
        "§cNo smoke item §f" + id + "§c. Try /ci smoke list."));
      return false;
    }
