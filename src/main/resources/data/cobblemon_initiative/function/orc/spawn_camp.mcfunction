...
# comes within 48 (chunk-load guard). Latch #orc_raised 0->1 FIRST (one-shot). Warband = 2 Orc
# Raiders + 1 Orc Marksman (a6: easy_npc:orc CROSSBOW — ranged, so block-stacking can't cheese the
# camp) + 1 Orc Warrior (easy_npc:orc_warrior) + 1 Orc War-Chief, import_new'd from
...
easy_npc preset import_new data easy_npc:preset/orc/orc_crossbow.npc.snbt ~3 ~1 ~-4
