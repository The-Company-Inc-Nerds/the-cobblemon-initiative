...
execute if score #stk_ok cd_calc matches 1.. run kill @e[tag=ci_anchor_sq_headcount_wager]
execute if score #stk_ok cd_calc matches 1.. at @s run summon minecraft:armor_stand ~1.5 ~ ~ {Invisible:1b,NoGravity:1b,Tags:["ci_anchor_sq_headcount_wager"]}
execute if score #stk_ok cd_calc matches 1.. run tbcs attach rctmod:sq_headcount_wager @e[tag=ci_anchor_sq_headcount_wager,limit=1]
execute if score #stk_ok cd_calc matches 1.. run tbcs battle GEN_9_SINGLES @s vs rctmod:sq_headcount_wager onwin {1: ['kill @e[tag=ci_anchor_sq_headcount_wager]', 'cobbledollars give @1 900', 'tag @1 add defeated_sq_headcount_wager', 'execute as @1 run function cobblemon_initiative:economy/wager_sweetener', '@2 say Documented. The purse is yours and the appendix is thicker for it.'], 2: ['kill @e[tag=ci_anchor_sq_headcount_wager]', '@1 say The stake goes to science. Payroll will find that hilarious.']}
