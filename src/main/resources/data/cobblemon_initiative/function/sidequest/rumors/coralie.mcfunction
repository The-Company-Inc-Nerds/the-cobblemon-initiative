# Nurse Lucia's rumor board - Gaviota Port's arrival hub. Run as the player from her
# "What is the word on the docks" dialog button. (a21: speaker renamed Coralie -> Lucia
# Marelli, the uuid-adopted gaviota_nurse; the fn PATH stays coralie so the compiled
# dialog button reference survives.) Rolls one of five town rumors on the
...
execute if score #rumor quest_hud matches 1 if entity @s[tag=!bosun_net_done] run tellraw @s [{"text":"Lucia, quietly: ","color":"aqua"},{"text":"Netmender Rui hung his nets to dry and will not fish. Bring the man deep-sea line - eight lengths - and he may remember the fleet owes you a lesson.","color":"gray"}]
...
execute if score #rumor quest_hud matches 2 if entity @s[tag=!odessa_crate_recovered] run tellraw @s [{"text":"Lucia, quietly: ","color":"aqua"},{"text":"There is a woman under the fish-market boards who sells what the tariff took. She wants a crate back off the customs float before she trusts you.","color":"gray"}]
...
execute if score #rumor_hit quest_hud matches 0 run tellraw @s [{"text":"Lucia, quietly: ","color":"aqua"},{"text":"Slow tide for gossip. The freight talk is all one word - Dunewind. The desert road east to Kalahar Reach, and everything the port cannot sell here goes that way now.","color":"gray"}]
